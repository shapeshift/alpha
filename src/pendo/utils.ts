/**
 * A no-import mechanism to convert a Uint8Array to a hex string.
 */
...
/**
 * A no-import mechanism to calculate a SHA-256 SRI value.
 */
export const toIntegrity = async (
  x: string | ArrayBuffer | ArrayBufferView
): Promise<`sha256-${string}`> => {
  if (typeof x === 'string') x = new TextEncoder().encode(x)
  const digest = new Uint8Array(await crypto.subtle.digest('SHA-256', x))
  return `sha256-${window.btoa(String.fromCharCode.apply(String.fromCharCode, Array.from(digest)))}`
}
